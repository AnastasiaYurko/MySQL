...
DROP DATABASE vk_with_posts_likes;
CREATE DATABASE vk_with_posts_likes;

-- Переходим в папку с дампом и вполняем загрузку
-- mysql -u root -p vk_with_posts_likes < vk_with_posts_likes.sql
...
-- исправляем недочеты в таблице posts
SELECT * FROM posts;
UPDATE posts SET community_id = FLOOR(1 + RAND() * 30) WHERE author_of_the_post = 'C';
UPDATE posts SET community_id = 0 WHERE author_of_the_post = 'U';
UPDATE posts SET user_id = FLOOR(1 + RAND() * 100) WHERE author_of_the_post = 'U';
UPDATE posts SET user_id = 0 WHERE author_of_the_post = 'C';
UPDATE posts SET comments_amount = 0 WHERE is_comments_closed = 1;
UPDATE posts SET media_type_id = FLOOR(1 + RAND() * 3);
